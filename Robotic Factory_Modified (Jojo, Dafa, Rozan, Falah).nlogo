...
to setup ; imports the base image from the folder, sets up the color of the factory, adds the processing machines, and robots.
  ...
  create-cutters 1 [ setxy 3 7  set color blue set shape "x" ]
  create-cutters 1 [ setxy 7 7 set color blue set shape "x" ]

  create-stitchers 1 [ setxy -3 -1 set color red set shape "box 2" ]
  create-stitchers 1 [ setxy 1 -1 set color red set shape "box 2" ]

  create-finishers 1 [ setxy -8 6 set color green set deluxe-level 1 set shape "circle" set processing-order? false ]
  create-finishers 1 [ setxy -4 6 set color green set deluxe-level 2 set shape "circle" set processing-order? false ]

  create-storages 1 [ setxy -14 2 set shape "box" set color magenta]

  create-robot-rooms 1 [ setxy -1 -7 set shape "box" set color yellow ]

  create-loading-docks 1 [ setxy 12 1 set shape "box" set color black set supply initial-supply set shape "box 2"]

  create-robots number-of-robots  [ ; creates a variable number of robots and sets their values
    ;setxy random-xcor random-ycor
    setxy -1 -7 ;; MODIFIKASI: datang dari robot rooms
    set color grey
...
to return-home
  ...
  ask robots [ if energy < 40 [ set destination one-of robot-rooms ] ]
end
...
to recharge
  ...
  ask robot-rooms [
    ...
    if sum [supply] of loading-docks > 0 [ ; if robots are needed for immediate tasks, interrupt some charging and send a robot.
      if any? robots-here [
        ask one-of robots-here with-max [energy] [
          ; note: this could be changed to a different heuristic, like selecting the robot
          ; with the most energy.
          ;if energy > just-in-time-supply-robot [ set destination one-of loading-docks ] ;; energy > just-in-time & ask one-of robots-here with max energy
...
to controller-sends-garments
  ask loading-docks [ set supply supply + demand]
end
...
to deliver-more-material-sheets ; 3% of the time, when the supply at the loading dock gets low, add some more supply to the loading dock.
  if random 100 < 3 [ if sum [supply] of loading-docks < demand [ controller-sends-garments ] ]
...
to label-agents-energy ; places tbhe labels on the agents if switched on
  ifelse show-energy? [
    ask robots [ set label round energy set label-color black] ;; show robots' energy
  ]
  [ ask turtles [ set label "" ] ]
end
